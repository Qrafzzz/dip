...
@section('content')
    ...
    <div class="flex flex-col md:flex-row md:items-start pt-5 p-80">
        ...
        <div>
            <h3>Отклики на Вашу вакансию</h3>
            @foreach($companyResumes as $companyResume)
                <div class="company border">
                    <div>
                        ...
                        <div>
                            {{$companyResume->resume->user->name}}<br>
                            {{$companyResume->resume->user->surname}}<br>
                        </div>
                        <div class="font-bold">
                            <a href="{{ route("resume.show", $companyResume->resume->author_id) }}">Подробнее</a>
                        </div>
                    </div>
                </div>
            @endforeach
