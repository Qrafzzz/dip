<div class="text-xl flex flex-col md:flex-row items-center justify-around px-4 md:px-8 py-2 md:py-4 bg-gradient-to-r from-bgblu to-bgfil">
    ...
    <div class="flex flex-col md:flex-row items-center justify-center mt-4 md:mt-0 text-lg sm:text-base">
        <form method="GET" action="{{ route('resume.index') }}" class="flex items-center">
            <input class="w-40 sm:w-48 md:w-52 lg:w-96 py-1 px-3 text-sm md:text-base lg:text-lg rounded-l-full" type="text" name="search" @if(isset($_GET['search'])) value="{{$_GET['search']}}" @endif placeholder="Поиск работников">
    ...
    <div class="flex flex-col md:flex-row items-center mt-4 md:mt-0 md:ml-4 text-lg sm:text-base justify-end">
        @guest('web')
            <a class="mr-4 text-sm md:text-base lg:text-lg" href="{{ route('registerWorker') }}">Разместить вакансию</a>
            <a href="{{ route('login') }}" class="text-sm md:text-base lg:text-lg">Войти</a>
        @endguest
        @auth('web')
            <a class="mr-4 text-sm md:text-base lg:text-lg" href="{{ route('profile') }}">Профиль</a>
            <a href="{{ route('logout') }}" class="text-sm md:text-base lg:text-lg">Выйти</a>
        @endauth
    </div>
</div>
