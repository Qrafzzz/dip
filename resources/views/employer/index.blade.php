...
@section('content')
    ...
    <div class="flex flex-row flex-wrap sm:flex-nowrap justify-center mt-5 mb-5">
        <div class="bg-white mr-3 px-4 pt-3 pb-4 sm:pb-0 hidden sm:pb-0 hidden sm:block" id="filter">
            <form method="GET" action="{{ route("employer.index") }}">
                @csrf
                <div class="">
                ...
                @if ($professions->count() > 5)
                    <div class="">
                        <button type="button" id="show-professions" class="text-blue-400 text-base">Показать остальные</button>
                    </div>
                @endif
                </div>
                <div class="">
                    ...
                    @if ($cities->count() > 5)
                        <div class="">
                            <button type="button" id="show-cities" class="text-blue-400 text-base">Показать остальные</button>
                        </div>
                    @endif
                </div>
                ...
                <div class="pt-3">
                    <button class="font-semibold" type="submit">Поиск</button>
                </div>
                    ...
                    @foreach($employers as $employer)
                        ...
                        <div class="flex pl-5 py-2 pr-3 border bg-white">
                            <div class="w-60 px-1 sm:mr-24 md:mr-10 lg:mr-80">
                                ...
                                <div>
                                    {{ $employer->min_wages }}
                                    @isset($employer->min_wages) - {{ $employer->max_wages }}
                                    @endisset
                                    <span class="font-bold">Руб.</span>
                                </div>
                                <div>{{ $employer->city->name }}</div>
                                <div>{{ $employer->name }}</div>
                            </div>
                            <div class="flex h-full w-20 md:w-24 lg:w-32">
                                <img class="m-auto" src="/storage/users/{{$employer->user->image}}">
                            </div>
                        </div>
                    @endforeach
